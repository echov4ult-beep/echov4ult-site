...
export function seedDatabase(db: DatabaseSync): void {
  ...
  for(const [key,value] of Object.entries({outreach_paused:true,generation_paused:true,agents_paused:false,global_automation_paused:true,delivery_paused:false,publishing_paused:false})) if(!db.prepare('SELECT 1 FROM settings WHERE key=?').get(key))setSetting(db,key,value);
